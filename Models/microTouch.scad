...
module cover() {
	...
	difference() {
		hull() {
			translate([29+t,bwid+t*2-fr,-pcbt])
		...
		difference() {
			...
			// screw tab slot for the mouting bolt. means you have to
			// print with support, but also acts as a spacer from the PCB.
			translate([-3, t+(bwid-tabw)/2-0.4,0]) cube([5.5,tabw,1]);
		}
		...
		translate([-3.5+5.5, (bwid+t*2)/2-0.4, -5])
			cylinder(d=3.5, h=20);
